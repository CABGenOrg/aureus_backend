...
router.post('/admin/users/register', async (req, res) =>{
	...
	try {
	   ...
	   exec("/bin/sh /cabgen/helper_scripts/notificarAdmin.sh", (error, stdout, stderr) => {
   		if (error) {
...
router.post('/admin/users/login', async (req, res) =>{
    ...
    const usuario = await Usuario.find( { usuario: params.usuario, senha: params.senha } , function(
    	...
    	if (err) {
	      console.log(err);
	      res.status(500).end(JSON.stringify({ estado: "error", mensaje: "Ha ocurrido un error interno/Internal Error" }));
        } else {
            if ( result[0] && result[0].estado && result[0].estado == 'ACTI' ) {
                ...
                res.status(401).end(JSON.stringify({ estado: "error", mensaje: "Usuario/Contrasena incorrecto. User/Password incorrect." }));
            }
        }
    });
});
...
router.post('/admin/sequences/list.json', function (req, res) {
    ...
    if (sess.usuario){
		...
		query = datatablesQuery(sequencia);
            	...
            	if (params.columns[15].search.value) {
    	    	    sequencia = sequencia.where('criadoPor').equals(params.columns[15].search.value);
    		}

	        var query = datatablesQuery(sequencia);
		query.run(params).then(function (data) {
